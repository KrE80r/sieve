...
class FeedSieve {
    ...
    renderMobileFilters() {
        ...
        categorySelect.innerHTML = '<option value="">All topics</option>' + cats.map(cat =>
            `<option value="${this.escapeHtml(cat)}">${this.escapeHtml(cat)} (${this.categories[cat]})</option>`
        ).join('');
    ...
    bindMobileFilters() {
        ...
        if (timeSelect) {
            timeSelect.value = this.timeFilter || 'today';
            timeSelect.addEventListener('change', (e) => {
                this.timeFilter = e.currentTarget.value;
                this.applyFilters();
            });
        }
        ...
        if (categorySelect) {
            categorySelect.addEventListener('change', (e) => {
                this.categoryFilter = e.currentTarget.value || null;
                this.applyFilters();
            });
        }
    ...
    bindSourceSheet() {
        ...
        const openSheet = () => {
            ...
            let html = `<button class="source-sheet-item source-sheet-all${this.sourceFilter ? '' : ' active'}" data-clear-source="true">
        ...
        listEl.addEventListener('click', (e) => {
            ...
            if (item.dataset.clearSource === 'true') {
                this.sourceFilter = null;
                this.updateSourceControl(null);
            ...
            this.applyFilters();
            closeSheet();
            this.updateSourceControl(sourceName);
        });
    ...
    updateSourceControl(sourceName) {
        const btn = document.getElementById('source-filter-btn');
        const label = btn?.querySelector('.source-filter-label');
        const visibleName = sourceName || 'All sources';

        if (label) label.textContent = visibleName;
        if (btn) btn.setAttribute('aria-label', `Filter by source, currently ${visibleName}`);
        if (sourceName) {
    ...
    render() {
        ...
        this.setupArticleImageFallbacks(list);
    ...
    articleReviewMarkup(review) {
        ...
        return `
            <header class="article-review-intro">
                ...
                <p>Open a source when it earns your attention, or simply move on. The inventory stays visible and has a real stopping point.</p>
    ...
    setupArticleImageFallbacks(list) {
        list.querySelectorAll('.article-source-visual img').forEach(image => {
            image.addEventListener('error', () => {
                const visual = image.closest('.article-source-visual');
                const article = image.closest('.article-item');
                const grid = image.closest('.article-card-grid');

                if (visual) visual.remove();
                article?.classList.remove('has-visual');
                article?.classList.add('no-visual');
                grid?.classList.remove('has-visual');
                grid?.classList.add('no-visual');
            }, { once: true });
        });
    }
    ...
    createArticle(item) {
        ...
        const rating = item.rating || null;
        const ratingBadgeHtml = rating ? this.createRatingBadgeHtml(rating, 'article-rating-badge') : '';
        const imageUrl = this.sourceImageUrl(item);
        const imageWidth = Number.parseInt(item.image_width, 10);
        const imageHeight = Number.parseInt(item.image_height, 10);
        const imageDimensions = Number.isInteger(imageWidth) && imageWidth > 0
            && Number.isInteger(imageHeight) && imageHeight > 0
            ? ` width="${imageWidth}" height="${imageHeight}"`
            : '';
        const imageMarkup = imageUrl ? `
                    <figure class="article-source-visual">
                        <img src="${this.escapeHtml(imageUrl)}" alt="${this.escapeHtml(item.image_alt || '')}"${imageDimensions} loading="lazy" decoding="async" fetchpriority="low" referrerpolicy="no-referrer">
                    </figure>` : '';

        return `
            <article class="article-item ${imageUrl ? 'has-visual' : 'no-visual'}" data-item-id="${item.id}">
                <div class="article-card-grid ${imageUrl ? 'has-visual' : 'no-visual'}">
                    <div class="article-card-copy">
                        <div class="article-overline-row">
                        ...
                        ` : ''}
                        <div class="article-footer" aria-label="Article actions for ${this.escapeHtml(item.title)}">
                            <a href="${this.escapeHtml(url)}" target="_blank" rel="noopener noreferrer" class="read-link" onclick="event.stopPropagation()">Read original ↗</a>
