...
class AwarenessView {
    ...
    cardMarkup(item) {
        ...
        const leadId = `development-${item.id}-lead`;
        ...
        const visualLink = item.external_links.find(link => link.preview_image_url);
        const visual = visualLink ? this.visualMarkup(visualLink) : '';
        return `
            <article class="development-dispatch${visual ? '' : ' no-visual'}${seen ? ' seen' : ''}"
                data-development-id="${item.id}" aria-labelledby="${leadId}">
                <div class="development-dispatch-copy">
                    ...
                    <p class="development-summary">
                        <strong class="development-lead" id="${leadId}">${this.escapeHtml(title)}</strong>
                        ${detail ? `<span class="development-detail">${this.escapeHtml(detail)}</span>` : ''}
                    </p>
                    <div class="development-provenance">
                        <span class="development-evidence ${item.evidence_state}">${evidenceLabel}</span>
                        <span>From selected lists</span>
                    </div>
    ...
    visualMarkup(link) {
        return `
            <figure class="development-visual">
                <img src="${this.escapeHtml(link.preview_image_url)}"
                    alt="${this.escapeHtml(link.title)}" loading="lazy" decoding="async"
                    referrerpolicy="no-referrer">
                <figcaption>
                    <span>Linked evidence</span>
                    <strong>${this.escapeHtml(link.domain)}</strong>
                </figcaption>
            </figure>
        `;
    }
    ...
    bindCards() {
        ...
        list.querySelectorAll('.development-visual img').forEach(image => {
            image.addEventListener('error', () => image.closest('.development-visual')?.remove(), { once: true });
        });
    }
