...
class AwarenessView {
    ...
    pruneExpiredDevelopments(now = this.currentTime()) {
        const active = this.developments.filter(
            item => new Date(item.expires_at).getTime() > now
        );
        if (active.length === this.developments.length) return false;
        this.developments = active;
        if (this.payload) this.payload.developments = active;
        return true;
    }
    ...
    scheduleExpiryCleanup() {
        ...
        if (!this.developments.length) return;

        const nextExpiry = Math.min(
            ...this.developments.map(item => new Date(item.expires_at).getTime())
        );
        const delay = Math.max(0, nextExpiry - this.currentTime());
        this.expiryTimer = window.setTimeout(() => {
            this.expiryTimer = null;
            if (this.pruneExpiredDevelopments()) {
                this.pruneSeenState();
                this.render();
                return;
            }
